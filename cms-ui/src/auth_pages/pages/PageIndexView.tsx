...
import PageSingleView from './PageSingleView';
...
import useSelectedPageAsState from './hooks/useSelectedPageAsState';

const TABLE_HEAD = ['Title', 'Slug', 'Order', 'Image', 'Status', ''];

const PageIndexView: React.FC = () => {
  ...
  const {
    ...
    data: page,
    setData: setPage,
    executeQuery: executePageQuery,
  } = useQuery<IResponseModel<IPost>>();

  const {
    loading: processingPageDelete,
    data: deletedPage,
    setData: setDeletedPage,
    executeDeleteCommand: executePageDeleteCommand,
  } = useDeleteCommand<IPost>();

  const {
    loading: processingPageUpdate,
    data: updatedPage,
    setData: setUpdatedPage,
    error: updatePageErrors,
    executeUpdateCommand: executePageUpdateCommand,
  } = useUpdateCommand<IPost>();

  const {
    loading: processingPageCreate,
    data: createdPage,
    setData: setCreatedPage,
    error: createPageErrors,
    executePostCommand: executePageCreateCommand,
  } = usePostCommand<IPost>();

  const {
    loading: processingPagePublish,
    data: publishedPage,
    setData: setPublishedPage,
    executePatchCommand: executePagePublishCommand,
  } = usePatchCommand<IPost>();

  const {
    loading: processingPageDraft,
    data: draftedPage,
    setData: setDraftedPage,
    executePatchCommand: executePageDraftCommand,
  } = usePatchCommand<IPost>();

  const {
    loading: loadingPages,
    data: pages,
    executeQuery: executeReloadPagesQuery,
  } = useQuery<IResponseModel<IPost[]>>();
  ...
  const pageRemoveActionHandler = (url: string) => {
    executePageDeleteCommand(url, HEADERS);
  };

  const pageUpdateActionHandler = (url: string) => {
    executePageUpdateCommand(url, JSON.stringify(selectedPageAsState), HEADERS);
  };

  const pagePublishActionHandler = (url: string) => {
    const pageStatus = { publishStatus: 'Published' };
    executePagePublishCommand(url, JSON.stringify(pageStatus), HEADERS);
  };

  const pageDraftActionHandler = (url: string) => {
    const pageStatus = { publishStatus: 'Draft' };
    executePageDraftCommand(url, JSON.stringify(pageStatus), HEADERS);
  };

  const pageCreateActionHandler = (url: string) => {
    executePageCreateCommand(
      url,
      JSON.stringify(createPageInputState),
      HEADERS
    );
  };

  useEffect(() => {
    executeReloadPagesQuery(
      `${process.env.REACT_APP_BASE_URL}/api/pages?page=${currentPage}&per_page=${recordPerPage}&search_text=${searchText}`,
      HEADERS
    );
    if (deletedPage) {
      setDeletedPage(null);
    ...
    if (createdPage) {
      ...
      Swal.fire('Created!', 'Your page has been created.', 'success');
    }

    if (updatedPage) {
      setUpdatedPage(null);
      setPageUpdateUrl(null);
      setSelectedPageAsState(null);
      Swal.fire('Updated!', 'Your page has been updated.', 'success');
    }
    if (publishedPage) {
      updateSelectedPageAsState('publishStatus', 'Published');
      setPublishedPage(null);
      Swal.fire('Published!', 'Your page has been published.', 'success');
      executePageQuery(pageViewUrl!, HEADERS);
    }
    if (draftedPage) {
      updateSelectedPageAsState('publishStatus', 'Draft');
      setDraftedPage(null);
      Swal.fire('Draft!', 'Your page has been drafted.', 'success');
      executePageQuery(pageViewUrl!, HEADERS);
    }

    if (page) {
      setSelectedPageAsState(page.data);
      setPage(null);
    }
  }, [
    page,
  ...
  return (
    <div className="">
      <Loading
        isLoading={
          loadingPages ||
          processingPageDelete ||
          processingPageCreate ||
          processingPageUpdate ||
          processingPagePublish ||
          processingPageDraft ||
          pageLoading
        }
      />
          ...
          <div className="relative flex flex-col overflow-hidden rounded shadow-sm hover:shadow">
            ...
            <div className="flex h-14 items-center justify-between border-b border-gray-200 bg-white px-4 dark:bg-blue-gray-900">
              <div className="flex items-center">
                ...
                <span className="ml-3 text-xl font-semibold">{`Page Details`}</span>
              </div>

              <div className="flex items-center gap-4">
                {selectedPageAsState?.links?.map((link: ILink) => {
                  if (
                    link?.rel === 'destroy' ||
                    link?.rel === 'publish' ||
                    link?.rel === 'draft'
                          ...
                          onClick={() => {
                            ...
                            if (link.rel === 'publish') {
                              Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                  pagePublishActionHandler(link.url);
                                }
                              });
                            }
                            if (link.rel === 'draft') {
            ...
            {/* Start of modal body */}
            <PageSingleView pageInputState={selectedPageAsState} />
            {/* Start of modal body */}
        ...
        <div className="max-h-screen w-full text-onSurface dark:text-gray-200">
          <div className="relative flex flex-col overflow-hidden rounded shadow-sm hover:shadow">
            ...
            <div className="flex h-14 items-center justify-between border-b border-gray-200 bg-white px-4 dark:bg-blue-gray-900">
              <div className="flex items-center">
                <i className="fa-solid fa-circle-info fa-lg"></i>
                <span className="ml-3 text-xl font-semibold">{`Edit Page`}</span>
              </div>
            ...
            {/* End of modal header */}
            <PageEditView
              pageInputState={selectedPageAsState}
              errors={updatePageErrors}
              updatePageInputState={updateSelectedPageAsState}
            />
            ...
            <div className="flex items-center justify-end gap-4 bg-surface py-2 px-4 dark:bg-blue-gray-900">
              <button
                onClick={() => {
                  pageUpdateActionHandler(pageUpdateUrl!);
                }}
      ...
      >
        <div className="max-h-screen w-full text-onSurface dark:text-gray-200">
          <div className="relative flex flex-col overflow-hidden rounded shadow-sm hover:shadow">
            ...
            {/* Start of modal footer */}
            <div className="flex items-center justify-end gap-4 bg-surface py-2 px-4 dark:bg-blue-gray-900 dark:bg-blue-gray-900">
              <button
                className="rounded bg-primary px-4 py-2 text-onPrimary shadow-sm transition-all duration-150 hover:scale-105 hover:bg-primaryVariant hover:shadow"
                onClick={() => {
                  pageCreateActionHandler(
                    pages?.createNew ? pages.createNew?.url : 'Unauthorized'
                  );
                }}
              >
                Submit
          ...
          <div className="mb-8 flex items-center justify-between gap-8">
            <div>
              ...
              <p className="mt-1 font-normal">
                See information about all pages
              </p>
            </div>
