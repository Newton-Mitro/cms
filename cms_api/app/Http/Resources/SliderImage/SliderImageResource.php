...
class SliderImageResource extends JsonResource
{
    ...
    public function toArray($request)
    {
        return [
            ...
            'order' => $this->order,
            'links' => [
                new LinkObject("index", "Posts", route('slider-images.index'), "GET", 'fa-solid fa-list-ul'),
                Auth::user()?->role === 'Super Admin' || Auth::user()?->role === 'Admin' || Auth::user()?->role === 'Content Manager' || Auth::user()?->role === 'Content Creator' ? new LinkObject("update", "Edit Slider Image", route('slider-images.update', $this->id), "PUT", 'fa-solid fa-pen-to-square') : null,
