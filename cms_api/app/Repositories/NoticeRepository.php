...
class NoticeRepository implements NoticeRepositoryInterface
{
    ...
    {
        return Notice::where('title', 'like', '%' . $searchText . '%')->orderBy('order', 'ASC')->paginate($perPage);
    }

    public function getPublicPosts()
    {
        $per_page = request()->filled('per_page') ? request()->query('per_page') : 10;
        return Notice::where('publish_status', 'Published')->paginate($per_page);
    }
    ...
    {
        return Notice::where('slug', $slug)->where('publish_status', 'Published')->get()->firstOrFail();
    }
