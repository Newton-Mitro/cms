...
class PostRepository implements PostRepositoryInterface
{
    ...
    {
        return Post::where('title', 'like', '%' . $searchText . '%')->orderBy('order', 'ASC')->paginate($perPage);
    }

    public function getPublicPosts()
    {
        $per_page = request()->filled('per_page') ? request()->query('per_page') : 10;
        return Post::where('publish_status', 'Published')->paginate($per_page);
    }
    ...
    {
        return Post::where('slug', $slug)->where('publish_status', 'Published')->get()->firstOrFail();
    }
